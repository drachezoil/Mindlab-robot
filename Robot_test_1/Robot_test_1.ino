...
#define switch1Pin    12  // switch
#define switch2Pin    13  // switch
...
void setup()
{
  ...
  pinMode(switch1Pin,INPUT);          // switch input
  pinMode(switch2Pin,INPUT);          // switch input
...
void loop()
{
  ...
  */
  ...
  else if (digitalRead(switch1Pin)==0 && digitalRead(switch2Pin)==0){
    motor (MOTOR_LEFT, MOTOR_STOP, PWM_FAST);
    motor (MOTOR_RIGHT, MOTOR_STOP, PWM_FAST);
  }
}
...
void motor (int motorName, int motorDir, int motorSpeed) {
  ...
  if (motorDir==1){  // direction = forward
    // set the motor speed and direction
    digitalWrite( motorName, HIGH ); // direction = forward
    analogWrite( motorName+1, 255-motorSpeed ); // PWM speed
  }
  else if (motorDir==0){  // direction = reverse
    // set the motor speed and direction
    digitalWrite( motorName, LOW ); // direction = reverse
    analogWrite( motorName+1, 255-motorSpeed ); // PWM speed
  }
  else if (motorDir==2){  // stop
    // set the motor speed and direction
    digitalWrite( motorName, HIGH );
    digitalWrite( motorName+1, HIGH );
  }
}
